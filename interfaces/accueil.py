...
def afficherPageAccueil(app,nomUser):
    frame = customtkinter.CTkFrame(app,width=1920,height=450,fg_color="transparent")
    frame.place(relx=0.5, rely=0.5, anchor="center")
    titre = customtkinter.CTkLabel(app, font=('Lota Grotesque', 60, 'bold', 'underline'))
    page(nomUser,titre)
    btnSalon = customtkinter.CTkButton(frame,text="Salons",image=iconSalon,command=lambda :typePieces(frame,titre.cget("text"),"Salon",app,nomUser),compound="top",hover=False,text_color="black",fg_color="transparent")
