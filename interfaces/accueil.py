...
def afficherPageAccueil(app,nomUser):
    ...
    page(nomUser,titre)
    btnSalon = customtkinter.CTkButton(frame,text="Salons",width=150,height=150,command=lambda :typePieces(frame,titre.cget("text"),"Salon",app,nomUser))
    btnSalon.place(relx=0.4, rely=0.1, anchor="n")
    btnCuisine=customtkinter.CTkButton(frame,text="Cuisines",width=150,height=150,command=lambda :typePieces(frame,titre.cget("text"),"Cuisine",app,nomUser))
    btnCuisine.place(relx=0.4, rely=0.8, anchor="s")
    btnChambre=customtkinter.CTkButton(frame,text="Chambres",width=150,height=150,command=lambda :typePieces(frame,titre.cget("text"),"Chambre",app,nomUser))
    btnChambre.place(relx=0.6, rely=0.1, anchor="n")
    btnSalBain = customtkinter.CTkButton(frame, text="Salles de bain", width=150, height=150,command=lambda :typePieces(frame,titre.cget("text"),"Salle de Bain",app,nomUser))
